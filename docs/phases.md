...
with a missing required field (e.g. no name) returns a 400 with a clear validation message, not a
500. Submitting a field the endpoint doesn't recognize (e.g. `role` on a product update) is silently
...
- Flagging the product as non-narcotic *after* this order exists does not change this order's
  `requiresVerification` value.
- Rejecting the prescription sets status to `rejected` and never allows it to reach `delivered`.

---

## Phase 16 — Payment Gateway Integration (Habib Metro)
**Goal:** FR-SYS-08/09 — real card payments, with the authorize/capture split for narcotics orders.
*(Exact implementation depends on the confirmed answers from the Habib Metro questionnaire — this
phase is scoped once those are in hand.)*
- `payments/providers/habibMetro.provider.js` behind the existing abstracted `payment.service.js` interface.
- Signature-verified webhook handler — never trust the browser redirect alone.
- Non-narcotics orders: charge immediately. Narcotics orders: authorize only, capture after approval (Phase 15).
- `gatewayTransactionId` stored on the order — never any card data.

**Test Case for Phase 16:** A standard order with card payment is charged and confirmed only after a
verified webhook call — a forged/unsigned webhook payload is rejected. A narcotics order with card
payment shows an authorization with no funds captured until an admin approves it in Phase 15's
workflow; rejecting it triggers a void, never a charge.

---

## Phase 17 — Order Cancellation & Refund
**Goal:** FR-AD-39 / FR-SYS-10 / FR-CW-24 — an admin can cancel an unshipped order (e.g. an
out-of-stock medicine), and the payment reverses correctly on its own.
- `PATCH /admin/orders/:id/cancel` — allowed only while status is Pending or Packed, optional reason.
- `order.service.js` branches on `paymentState`: COD → nothing to reverse; authorized-only → void via
  the gateway; captured → refund via the gateway using `gatewayTransactionId` (Phase 16).
- Cancellation writes an Activity Log entry and sends the customer a single cancellation email,
  distinct from the order-confirmation email.

**Test Case for Phase 17:** Cancel a Pending COD order — status becomes Cancelled, no payment call is
made, one cancellation email is sent. Cancel a Pending narcotics order that was only authorized — the
gateway records a void, never a capture. Cancel a Packed standard order that was already charged — a
refund call is made with the correct transaction reference. Attempt to cancel a Shipped order — the
API rejects it (this flow is for pre-fulfillment cancellation only, not post-shipment returns).
...
the *global* limiter applies even where nothing sensitive is happening. Hammer the OTP endpoint past
