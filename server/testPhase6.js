...
async function runTests() {
  ...
  await assert("Real admin email → 200 (generic message)", async () => {
  ...
  await assert("passwordReset record created in DB with correct fields", async () => {
    ...
    expectTruthy(record.expiresAt > new Date(), "Token expiry must be in the future");
    expectTruthy(!record.used, "Token must not be used yet");
    ...
    // Inject a known test token so we can verify the reset flow without reading email
    ...
    rawToken = freshRaw;
    console.log(`       [TEST] Injected test token into DB record for reset verification`);
  });
  ...
  await assert("Admin can log in with the NEW password after reset", async () => {
    ...
    expectTruthy(body.data?.token, "Must return a JWT token");
  });

  // ── TEST 3: Reuse the same token ──────────────────────────────────────────
  console.log("\n🚫  Test 3: Reuse same token after consumption");
